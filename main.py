...
secret = 'AdItYaDaHiYa123'

def render_str(template, **params):
    t = jinja_env.get_template(template)
    return t.render(params)

# Obtain key from the blog table
...
class PostPage(BlogHandler):
    ...
    def post(self, blog_id):
        ...
        # an error
                    ...
                    comment_error = "Please enter a comment in the textarea"
                    self.render(
                        "post.html",
                        post=post,
                        likes=likes,
...
class Comment(db.Model):
    post = db.ReferenceProperty(Post, required=True)
    user = db.ReferenceProperty(User, required=True, default=None)
    created = db.DateTimeProperty(auto_now_add=True)
...
class Logout(BlogHandler):
    ...
    def get(self):
        ...
        # page and throw an error
            ...
            error = 'You need to be logged in to be able to log out.'
            self.render('login.html', error=error)
